...
if ($arr[2] != "")
{$serial = $arr[2];}
$query="select top 100 row = row_number() over ( order by tTime), temp from battery_data t1 join state_val_lookup t2
...
?>
<script> // variables
var canvas= ['ChartS0','ChartS1','ChartS2','ChartS3','ChartS4','ChartS5','ChartS6'];
...
<canvas id= canvas[station] ></canvas>
<script>
if (getReq2 >= 1){
			...
			while( $info = SQLSRV_FETCH_ARRAY($stmt,SQLSRV_FETCH_ASSOC))
			 ...
			 Temps.reverse();
			  <?php
			 $query="select top 100 row = row_number() over ( order by tTime), curr from battery_data t1 join state_val_lookup t2
on t1.state = t2.state
where model = 
...
$stmt=SQLSRV_QUERY($conn,$query);
			...
			while( $info = SQLSRV_FETCH_ARRAY($stmt,SQLSRV_FETCH_ASSOC))
			  ...
			  Currs.reverse();
			 <?php
			 $query="select top 100 row = row_number() over ( order by tTime), volt from battery_data t1 join state_val_lookup t2
on t1.state = t2.state
where model = 
'".$model."' and serial = '".$serial."' and t1.state = any (select state from STATE_VAL_LOOKUP) 
order by row desc";
$stmt=SQLSRV_QUERY($conn,$query);
			...
			Volts.reverse();
<?php
			 $query="select top 100 row = row_number() over ( order by tTime), tTime from battery_data t1 join state_val_lookup t2
on t1.state = t2.state
where model = 
'".$model."' and serial = '".$serial."' and t1.state = any (select state from STATE_VAL_LOOKUP) 
order by row desc";
